...
import { DepAcctActivityInfoInqPayloads, ApiHeaders } from '../../src/fixtures/apiPayloads';
...
describe('DepAcctActivityInfoInq', () => {
  ...
  test('Inquire deposit account activity information with time range', async () => {
    const payload = DepAcctActivityInfoInqPayloads.depAcctActivityInfoInqPayload();
    const headers = ApiHeaders.getHeaders_STEL_SIT();
    const requestId = headers['X-Request-ID'];

    try {
      // ====== STEP 1: Call API ======
      console.log(`\n[Step 1] Calling DepAcctActivityInfoInq API with Request ID: ${requestId}`);
      const response = await apiClient.post(
        '/v1/proxy-gateway/payment/teller/DepAcctActivityInfoInq',
        payload,
        { headers }
      );
      ...
      expect(payload.productCode).toBe('STEL');
