...
import { PaySvcPayloads, ApiHeaders } from '../../src/fixtures/apiPayloads';
...
describe('PaySvc', () => {
  ...
  describe('Verify', () => {
    describe('Channel ENET', () => {
      test('Verify Bill Payment via ENET with Atomic SUN', async () => {
        const headers = ApiHeaders.getHeaders_ENET_SIT();
        const requestId = headers.correlationId;
        const payload = PaySvcPayloads.verifyBillPaymentViaENETWithAtomicSUN(requestId);

        try {
          // ====== STEP 1: Call API ======
          console.log(`\n[Step 1] Calling PaySvc Verify API with Request ID: ${requestId}`);
          const response = await apiClient.post(
            '/v1/proxy-gateway/payment/paysvc',
            payload,
            { headers }
          );
          ...
          expect(payload.TranInfo.TranType).toBe('PAYMENTVERIFY');
          expect(payload.CustId.SPName).toBe('ENET');
    ...
    describe('Channel ATMI', () => {
      // Channel ATMI test cases will be added here
    });
  });

  describe('Confirm', () => {
    // Confirm test cases will be added here
  });
});
